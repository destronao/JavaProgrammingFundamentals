...
Esta clase debe tener (el nombre de cada método va entre paréntesis):
...
- Un método que traslade dx y dy unidades el punto (trasladar). dx y dy serán pasados como parámetros.  
  Ejemplo: si un punto p tiene las coordenadas (1,2) → p(1,2) y se llama al método trasladar con los valores (2,-5), moverá el punto p a las coordenadas (3,-3) (x+dx, y+dy).
...
2. Crea una nueva clase con método main (recuerda establecer el nombre según las instrucciones al comienzo del boletín: Iniciales de tus apellidos …) y realiza lo siguiente:
...
- Crea una variable «x» y guarda en ella la coordenada x del punto p, luego muestra el valor de «x».
- Crea una variable «y» y guarda en ella la coordenada y del punto p, luego muestra el valor de «y».
- Crea un nuevo punto p2 y asígnale las coordenadas de las variables x+1 e y+1 creadas anteriormente.
- Determina si los punto p y p2 son iguales.
...
Crea una nueva clase con método main():
...
Define un clase `MatesXY` que permita trabajar con 2 números (x,y) enteros (int). Sin utilizar la clase Math.
Se pide que permita realizar y devolver el resultado de las operaciones planteadas. Además de devolver el resultado, debe también mostrar por pantalla un mensaje con la operación realizada y dicho resultado.

Consideración 1: no será necesario crear objetos de esta clase para trabajar con ella.  
Consideración 2: el usuario puede introducir (dentro de los números enteros) cualquier valor. Será nuestra responsabilidad controlar posibles errores.  
Consideración 3: ningún método debe tener parámetros.  
Consideración 4: inicialmente ambos enteros tomarán el valor 0.
...
// El método sumar(), además de devolver el resultado, muestra:
// «1+(-2) = -1»

Crear una clase con método main que trabaje con dicha clase.
Utiliza, al menos, una vez cada una de las operaciones.
...
- Mostrar un mensaje al sacar 700€ indicando si se pudo o no realizar dicha operación.
...
- Método prestamo: incrementa el atributo correspondiente cada vez que se realice un préstamo del libro.  
...
- Método devolucion: decrementa el atributo correspondiente cuando se produzca la devolución de un libro.  
  No se podrán devolver libros que no se hayan prestado.  
  Devuelve true si se ha podido realizar la operación y false en caso contrario.
- Método toString para mostrar los datos de los libros.

Escribe una aplicación para probar el funcionamiento de la clase Libro:
- Crear un libro libro1 con los valores: El Quijote, Cervantes, 1 ejemplar y 0 prestados.
- Crear un libro libro2 con los siguientes valores introduciéndolos por teclado:
  - Titulo: Problemas de Java
  - Autor: Jose María Pérez
  - Ejemplares: 4
- Realizar préstamos del libro1 y devoluciones comprobando que no se puede prestar el libro cuando no quedan ejemplares y prestándolo siempre que prestados < número de ejemplares.  
  Mostrar los mensajes necesarios y el valor de los atributos del libro1 para comprobar que funciona.
...
Define una clase `CuentaCorriente2`. Copia el código de la actividad 4 y realiza las modificaciones necesarias para que todo el código sea válido en esta nueva clase. Además:
...
- Modificar la clase CuentaCorriente2 para que pueda disponer de un objeto Gestor.
...
5. Si come o duerme en exceso y su nivel se pone por encima de 50 se pone enfermo, y lo mismo ocurre cuando hace demasiado ejercicio y su nivel baja de 15.
   - Además un perro puede ponerse enfermo si la comida le sienta mal: cuando come hay una probabilidad de 3/10 de que la comida le siente mal.
6. El perro virtual se comunica con su dueño cada vez que hace una actividad, indicándole:
...
7. Se puede controlar el nivel de energía cuando se quiera.
...
- Cada objeto de esta clase representará una hucha electrónica, que sólo puede contener monedas de 1€, de 2€, o billetes de 5€.
...
1. Constructor: recibirá un valor que servirá para fijar si la hucha está cerrada o no.
...
Crea una clase con método main que realice un recorrido por las diferentes posibilidades existentes presentando un menú de opciones donde el usuario podrá elegir la opción deseada.
